...
interface Props {
  ...
  compAddress: string;
  setCompAddress: (val: string) => void;
  onAdd: () => void;
  onDelete: (id: number) => void;
}

export const CompanyManager = ({ 
  companies, compName, setCompName, compAddress, setCompAddress, onAdd, onDelete 
}: Props) => (
  <View style={{ flex: 1 }}>
    <View style={styles.formCard}>
      <Text style={styles.formTitle}>Thêm Công Ty Mới</Text>
      <TextInput style={styles.input} placeholder="Tên công ty" value={compName} onChangeText={setCompName} />
      <TextInput style={styles.input} placeholder="Địa chỉ" value={compAddress} onChangeText={setCompAddress} />
      <TouchableOpacity style={styles.btnPrimary} onPress={onAdd}>
        <Text style={styles.btnPrimaryText}>LƯU CÔNG TY</Text>
      </TouchableOpacity>
    </View>
      ...
      renderItem={({ item }) => (
        <View style={styles.dataCard}>
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.cardSub}>ID: {item.id} • {item.address}</Text>
          </View>
